...
const CompanyReschedulePenaltyLogRow = ({ event, classes }) => {
  const { invoiceItem, createdAt } = event;
  ...
  return (
    <div key={event.id} style={{ margin: 5 }}>
      <h4 className={classes.eventText}>
        {translations.t('shootingEvents.companyRescheduledPenaltyMessage', {
...
export default _.flow([withStyles(styles), connect()])(CompanyReschedulePenaltyLogRow);
