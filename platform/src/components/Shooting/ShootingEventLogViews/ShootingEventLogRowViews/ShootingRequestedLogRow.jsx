...
const ShootingRequestedLogRow = ({ shooting, isBoom, event, classes }) => {
  ...
  return (
    <div key={event.id} style={{ margin: 5, display: 'flex' }}>
      <h4 className={classes.eventText}>
        {translations.t('shootingEvents.shootingRequestedMessage', { subject })}
        ...
        {!shooting.startDate && <span>&nbsp;{translations.t('general.withoutDate')}</span>}
      </h4>
    </div>
  );
};

export default _.flow([withStyles(styles), connect()])(ShootingRequestedLogRow);
