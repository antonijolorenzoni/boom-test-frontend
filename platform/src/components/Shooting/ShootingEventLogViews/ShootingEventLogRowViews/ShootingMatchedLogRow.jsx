...
const ShootingMatchedLogRow = ({ event, classes }) => {
  const { createdAt } = event;
  return (
    <div key={event.id} style={{ margin: 5 }}>
      <h4 className={classes.eventText}>
        {translations.t('shootingEvents.shootingMatchedMessage')}
...
export default _.flow([withStyles(styles), connect()])(ShootingMatchedLogRow);
