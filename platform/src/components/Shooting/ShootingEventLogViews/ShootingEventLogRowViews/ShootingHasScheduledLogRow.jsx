  ...
  return (
    <div key={event.id} style={{ margin: 5 }}>
      <h4 className={classes.eventText}>
        {translations.t('shootingEvents.shootingHasScheduledMessage', {
          subject: getUserName(),
          actor: translations.t(`scheduleActors.${source}`),
          date: moment(shootingDate).format('L'),
          hour: moment(shootingDate).format('HH:mm'),
        })}
        {`, `}
